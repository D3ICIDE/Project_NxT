    ...
    public static void handleInteraction(
        ...
        try {
            ...
            while (iterationCount < MAX_TOOL_ITERATIONS && !interactionComplete) {
                ...
                // 1. Call the LLM with the current state of the history
                ...
                // 2. Did the LLM decide to use tools?
                if (messageObj.has("tool_calls") && !messageObj.get("tool_calls").isJsonNull()) {
                    ...
                    // 4. Wait for all tools to finish
                    ...
                    // Clear the user message so the next loop pass only evaluates the tool results
